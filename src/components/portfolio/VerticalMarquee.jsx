...
const MarqueeColumn = ({ items, direction }) => {
    ...
    return (<div className="relative h-[420px] sm:h-[520px] lg:h-[600px] overflow-hidden">
      <motion.div className={`flex flex-col gap-4 ${direction === 'up' ? 'animate-marquee-up' : 'animate-marquee-down'}`} style={{ willChange: 'transform' }}>
        {duplicatedItems.map((src, index) => (<motion.div key={index} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }} className="relative group">
            <div className="w-40 h-28 sm:w-52 sm:h-36 md:w-64 md:h-48 rounded-2xl overflow-hidden shadow-card hover-lift">
              <img src={src} alt={`Portfolio item ${index + 1}`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"/>
...
const VerticalMarquee = () => {
    ...
    return (<section className="py-14 sm:py-20 md:py-24 bg-secondary overflow-hidden">
