...
const Navbar = () => {
    ...
    return (<>
      <motion.nav initial={{ y: -100 }} animate={{ y: 0 }} transition={{ duration: 0.6 }} className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled
            ? 'py-2 sm:py-4 glass shadow-soft'
        ...
        <div className="container mx-auto px-4 flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center">
            <div className={`rounded-md px-2 py-1 transition-all ${isScrolled ? 'bg-transparent' : 'bg-white/10 backdrop-blur-sm'}`}>
              <img src={heightsLogo} alt="Heights Logo" className={`h-8 sm:h-10 md:h-11 w-auto object-contain transition-all ${isScrolled ? '' : 'mix-blend-multiply'}`}/>
            </div>
          </Link>
      ...
      {/* Mobile Menu */}
      <AnimatePresence>
        {isMobileMenuOpen && (<motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="fixed inset-x-0 top-14 sm:top-[72px] z-40 bg-card shadow-elevated md:hidden">
