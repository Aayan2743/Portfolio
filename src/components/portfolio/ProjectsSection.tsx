...
const ProjectsSection = () => {
  ...
  const [modalState, setModalState] = useState<{
    ...
    projectId: string;
  }>({ isOpen: false, type: 'interested', projectId: '' });

  const filteredProjects = activeCategory
    ? projects.filter(p => p.categoryId === activeCategory)
    : projects;

  const openModal = (projectId: string, type: 'interested' | 'documented') => {
    setModalState({ isOpen: true, type, projectId });
  };
            ...
            {filteredProjects.map((project, index) => (
              ...
              >
                <ProjectCard
                  project={project}
                  index={index}
                  onInterested={() => openModal(project.id, 'interested')}
                  onDocument={() => openModal(project.id, 'documented')}
                />
              </motion.div>
            ))}
          </AnimatePresence>
