...
interface AuthContextType {
  ...
  userRegister: (email: string, password: string, name: string) => Promise<boolean>;
  userLogout: () => void;
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const userLogin = async (email: string, password: string): Promise<boolean> => {
    ...
    const passwords: Record<string, string> = JSON.parse(localStorage.getItem(passwordsKey) || '{}');
  ...
  const userRegister = async (email: string, password: string, name: string): Promise<boolean> => {
    await new Promise(resolve => setTimeout(resolve, 500));
