    ...
    return (<div className="space-y-8">
      ...
      <div className="flex flex-wrap gap-4">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input placeholder="Search by mobile number..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10"/>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-2xl p-6 shadow-card">
      ...
      <div className="bg-card rounded-2xl shadow-card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filteredInteractions.length === 0 ? (<tr>
                  <td colSpan={4} className="text-center py-12 text-muted-foreground">
                    No interactions found
                  </td>
                </tr>) : (filteredInteractions.map((interaction, index) => {
            const project = projects.find(p => p.id === interaction.projectId);
            return (<motion.tr key={interaction.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} className="border-b border-border last:border-0">
